...
export default function KeyPoints() {
  ...
  return (
    <div className="max-w-screen-xl mx-auto md:mb-6">
      ...
      <div className="grid grid-cols-1 gap-y-6 md:grid-cols-2 md:gap-20 items-start">
        {sections.map((item) => (
          <React.Fragment key={item.id}>
            {item.type === "sandbox" ? (
              // sandbox Scholar Layout
              <div className="bg-[var(--secondary-background)] border-3 md:border-y-3 md:border-e-3 border-[var(--border-color)] px-3 md:px-5 rounded-sm md:rounded-tr-sm md:rounded-br-sm md:-mt-[20%] md:max-w-[95%] mx-5 md:mx-auto">
                {/* Title */}
                <p className="font-playfair-boldItalic text-3xl md:text-5xl pb-1 py-2">
                  {item.title}
                </p>
                ...
                <div className="md:flex justify-evenly mb-5">
                  <div className="space-y-2 md:space-y-0">
                    <p className="text-[var(--primary-color)] font-neue-ultraBold text-2xl sm:text-3xl md:text-4xl leading-[19px] md:leading-8">
                      {item.quoteTitle}
                    </p>
                    <div className="flex mt-1">
                      <p className="font-zawi-bold uppercase md:text-lg text-[var(--foreground-color)]/78">
                        {item.quoteSubtitle}
                      </p>
                      <sup className="hidden sm:block">
                        <ImQuotesRight className="md:size-6 ms-1" />
                      </sup>
                    </div>
                  </div>
                  {/* Decorative hash symbol */}
                  <p className="font-rig-bold md:text-8xl me-3 hidden md:block">
                    #
                  </p>
                </div>
              </div>
              ...
              <div className="sm:flex sm:justify-center sm:space-x-6 sm:items-center bg-[var(--primary-color)]/15 rounded-4xl p-6 md:py-8 max-w-[95%] mx-auto mt-auto">
                {/* KeyPoint icon */}
                <img
                  src={`/images/${item.img_name}.svg`}
                  alt={item.img_name}
                  className="w-24 object-contain mx-auto mb-2 sm:mb-0 sm:mr-4"
                />
            ...
            {item.type === "sandbox" && (
              <p className="md:hidden text-[var(--foreground-color)] md:text-[17px] text-justify px-4 mb-4">
